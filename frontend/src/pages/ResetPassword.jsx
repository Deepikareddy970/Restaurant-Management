  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setError(data.message || 'Failed to reset password. Please check the details.');
      }
    } catch (err) {
        ...
        <form onSubmit={handleSubmit} className="space-y-4 relative z-10">
          ...
          <div>
            ...
            <input
              ...
              required
              maxLength={6}
              className="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-800 bg-transparent text-center text-lg font-bold tracking-widest text-slate-850 dark:text-slate-100 placeholder-slate-350 focus:outline-none focus:border-indigo-600 transition duration-200"
              placeholder="000000"
              value={otpCode}
          ...
          <div>
            ...
            <input
              type="password"
              required
              className="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-800 bg-transparent text-slate-850 dark:text-slate-100 placeholder-slate-450 focus:outline-none focus:border-indigo-600 transition duration-200 text-sm"
              placeholder="••••••••"
              value={newPassword}
